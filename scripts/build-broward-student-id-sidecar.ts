/**
 * Build PyInstaller du sidecar broward-student-id pour Tauri externalBin.
 */
...
const sidecarDir: string = path.join(rootDir, 'sidecar', 'broward-student-id')
...
console.log(`Building broward-student-id sidecar for ${targetTriple}...`)
...
if (!pythonBin) {
  console.error('Python not found. Install Python 3.14.5 then run: npm run sidecar:build:student-id')
  process.exit(1)
}
...
if (!pythonVersion.includes('3.14.5')) {
  console.error(`Python 3.14.5 required for the student-id sidecar. Found: ${pythonVersion}`)
  process.exit(1)
}
...
execSync(`${pythonBin} -m PyInstaller --noconfirm broward-student-id.spec`, {
  cwd: sidecarDir,
  stdio: 'inherit',
})

const builtPath: string = path.join(distDir, `broward-student-id${extension}`)
const destPath: string = path.join(binariesDir, `broward-student-id-${targetTriple}${extension}`)
